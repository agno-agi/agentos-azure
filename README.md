...
2. **AgentOS UI.** Chat with agents, build new ones, and inspect sessions, traces, memory, and evals from the AgentOS UI at [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-azure&utm_content=agentos-azure&utm_term=azure).
...
```sh
git clone https://github.com/agno-agi/agentos-azure agentos
cd agentos
...
1. Open [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-azure&utm_content=agentos-azure&utm_term=azure) and sign in.
...
You can run the platform anywhere that supports containerized images. This template deploys to [Azure Container Apps](https://azure.microsoft.com/products/container-apps) with a single script — Postgres Flexible Server on a private VNet, secrets in Container Apps, always-on single replica — and a coding-agent skill, [`/deploy-platform`](.agents/skills/deploy-platform/SKILL.md), that drives it for you and verifies the live platform at the end.
...
During `./scripts/azure/up.sh`, once the app URL exists the script pauses so you can mint the key.

1. Open [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-azure&utm_content=agentos-azure&utm_term=azure), click **Connect OS** → **Live**, and enter your Container Apps URL.
...
If you run non-interactively or skip the prompt, you can sync environment variables later with `./scripts/azure/env-sync.sh`.
...
```sh
uvx agno connect --url https://<your-container-apps-url>
```

For **claude.ai and ChatGPT (web)**: add `https://<your-container-apps-url>/mcp` as a custom connector in the chat app's connector settings. Leave the form's optional OAuth fields (client ID / client secret) empty. Click **Connect** and, on the consent page, enter the `MCP_CONNECT_SECRET` that `up.sh` generated during deploy (saved in `.env.production`).
...
Rebuilds the image locally, pushes it to the registry, and rolls the app with a fresh revision suffix.
...
Secret-shaped keys become Container Apps secrets; everything else becomes plain env vars. One revision roll applies it all.
...
Deletes the **entire resource group** — app, environment, Postgres (all data), registry, network. up.sh creates a dedicated group precisely so teardown is this complete; the script lists the group's resources before asking for confirmation.
...
| `AGENTOS_URL` | no | `http://127.0.0.1:8000` | Scheduler base URL. `scripts/azure/up.sh` sets it to the Container Apps URL right after the first deploy (a second revision — the FQDN is only known post-create) and writes it back into your env file. Also the public origin OAuth metadata derives from when `MCP_CONNECT_SECRET` is set. |
...
- [Agno documentation](https://docs.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-azure&utm_content=agentos-azure&utm_term=azure)
- [AgentOS introduction](https://docs.agno.com/agent-os/introduction?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-azure&utm_content=agentos-azure&utm_term=azure)
